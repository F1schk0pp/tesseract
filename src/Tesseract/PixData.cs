...
namespace Tesseract
{
    public unsafe class PixData
    {
        ...
        /// </remarks>
        public void EndianByteSwap() => Interop.LeptonicaApi.Native.pixEndianByteSwap(Pix.Handle);
        ...
        public static uint GetDataByte(uint* data, int index) =>
            ...
            IntPtr.Size == 8 ? *((byte*)((ulong)((byte*)data + index) ^ 3)) : *((byte*)((uint)((byte*)data + index) ^ 3));// Architecture types that are NOT little edian are not currently supported//return *((byte*)data + index);  
        ...
        public static uint GetDataTwoByte(uint* data, int index) =>
            // Must do direct size comparison to detect x64 process, since in this will be jited out and results in a lot faster code (e.g. 6x faster for image conversion)
            IntPtr.Size == 8 ? *(ushort*)((ulong)((ushort*)data + index) ^ 2) : *(ushort*)((uint)((ushort*)data + index) ^ 2);// Architecture types that are NOT little edian are not currently supported// return *((ushort*)data + index);
