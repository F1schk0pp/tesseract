...
namespace Tesseract
{
    ...
    public class PixArray : DisposableBase, IEnumerable<Pix>
    {
        ...
        public bool Add(Pix pix, PixArrayAccessType copyflag = PixArrayAccessType.Clone)
        {
            ...
            Guard.Require(nameof(copyflag), copyflag is PixArrayAccessType.Clone or PixArrayAccessType.Copy,
        ...
        {
            Guard.Require(nameof(accessType), accessType is PixArrayAccessType.Clone or PixArrayAccessType.Copy, "Access type must be either copy or clone but was {0}.", accessType);
            Guard.Require(nameof(index), index >= 0 && index < Count, "The index {0} must be between 0 and {1}.", index, Count);
